...
local ArkSkillDesc = require "widgets/ark_skill_desc_ling"

-- 闪烁效果配置
local BLINK_DURATION = 0.5  -- 单次闪烁持续时间（秒）

local SKILL_WIDTH = 128
...
local ArkSkill = Class(Widget, function(self, owner, config, idx)
  ...
  self.config = config
  self.idx = idx
  self.size = {SKILL_WIDTH, SKILL_WIDTH}
  ...
  -- 设置绿色半透明
  chargeShadow:SetTint(0, 1, 0, 0.5)
  ...
  -- 设置橘黄色半透明
  buffShadow:SetTint(1, 0.5, 0, 0.5)
...
function ArkSkill:OnUpdate(dt)
  SendModRPCToServer(GetModRPC("arkSkill_ling", "RequestSyncSkillStatus"), self.idx)
...
function ArkSkill:OnGainFocus()
  ...
  if not self.skillDesc then
    local descConfig = {
      ...
      lockedDesc = self.config.lockedDesc or "",
...
function ArkSkill:TryActivateSkill()
  ...
  if self.config.bullet and self.status == CONSTANTS.SKILL_STATUS.BULLETING then
    SendModRPCToServer(GetModRPC("arkSkill_ling", "ManualCancelSkill"), self.idx)
    return
  end
  SendModRPCToServer(GetModRPC("arkSkill_ling", "ManualActivateSkill"), self.idx, TheInput:IsKeyDown(KEY_CTRL) or TheInput:IsKeyDown(KEY_RCTRL))
end
