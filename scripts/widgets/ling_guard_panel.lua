...
-- 将字符串转换为竖排（每字符一行），支持UTF-8
local function ToVerticalString(str)
...
local LingGuardPanel = Class(Widget, function(self, owner)
  ...
  -- 名称变更（namedirty）时刷新文本
  self._on_namedirty_proxy = function()
    self:UpdateGuardNameText()
  end
...
function LingGuardPanel:AttachGuard(inst)
  ...
  if inst and inst:IsValid() then
    ...
    inst:ListenForEvent("namedirty", self._on_namedirty_proxy, inst)
...
function LingGuardPanel:DetachGuard()
  ...
  if inst and inst:IsValid() then
    ...
    inst:RemoveEventCallback("namedirty", self._on_namedirty_proxy, inst)
...
function LingGuardPanel:SetRenameCharSpacing(spacing)
  if spacing and spacing > 0 then
    self.rename_char_spacing = spacing
    self:UpdateGuardNameText()
  end
end

function LingGuardPanel:UpdateGuardNameText()
  if not self.rename_text_root then return end
  local inst = self.guard_inst
  local name = ""
  if inst and inst:IsValid() and inst.replica and inst.replica.named and inst.replica.named._name then
    name = inst.replica.named._name:value() or ""
  end
  ...
  local spacing = self.rename_char_spacing or math.floor((self.rename_font_size or 60) * 0.8)
