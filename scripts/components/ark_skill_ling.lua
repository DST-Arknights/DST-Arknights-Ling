...
function ArkSkill:SetupSkillConfig(prefab)
  local config = TUNING.ARK_SKILL[string.upper(prefab)]
  self.config = config
  -- 搞几个默认值进去
...
function ArkSkill:SyncSkillStatus(idx, sendEvent)
  if sendEvent == nil then
    sendEvent = true
  end
  local data = self:GetSkillData(idx)
  if self.clientConnected then
    SendModRPCToClient(GetClientModRPC("arkSkill_ling", "SyncSkillStatus"), self.inst.userid, idx, data.status, data.level,
      data.energyProgress, data.buffProgress, data.bullet, data.activationStacks)
  end
  if sendEvent and self.onSkillStatusChange then
...
function ArkSkill:OnUpdate(dt)
  for i = 1, #self.skills do
    -- 如果不是处于锁定状态
    if self:GetSkillData(i).status ~= CONSTANTS.SKILL_STATUS.LOCKED then
      -- buff流动期间, 不会自动时间充能
...
function ArkSkill:GetActivationStacks(idx)
  return self:GetSkillData(idx).activationStacks
end

-- 技能状态查询接口
function ArkSkill:IsSkillActive(idx)
  return self:GetSkillData(idx).status == CONSTANTS.SKILL_STATUS.BUFFING
end

function ArkSkill:GetSkillStatus(idx)
  return self:GetSkillData(idx).status
end
