...
local function find_ents_near_leader_cached(leader, radius)
    ...
    if entry == nil then
        entry = { radii = {}, listener_added = false }
    ...
    if rec == nil or (now - rec.time) > CACHE_TTL then
        ...
        rec = { time = now, pos = pos, ents = ents }
...
local function IsThreatToGuard(inst, guy)
    ...
    if guy:HasTag("companion") then return false end
    if guy:HasTag("wall") then return false end
...
local function intersect_lists(a, b)
    ...
    for _, v in ipairs(a) do
        if setb[v] then table.insert(res, v) end
    end
    return res
end

local function find_ents_at(pos, radius)
    if not pos or not radius or radius <= 0 then return {} end
...
local function SelectTarget(inst)
    if inst == nil or inst:IsInLimbo() then return nil end

    local cur, common, mode = GetModeConfig(inst)
    local leader = inst.components.follower and inst.components.follower.leader or nil

    local self_chase = (cur and cur.ATTACK and cur.ATTACK.CHASE_DIST) or 8
    local leader_def = (common and common.LEADER_DEFENSE_DIST) or 8

    local candidates = nil

    if mode == CONSTANTS.GUARD_BEHAVIOR_MODE.GUARD then
        local guard_range = (cur and cur.GUARD_RANGE) or 16
        local guard_pos = (inst.components.ling_guard and inst.components.ling_guard.guard_pos) or (leader and leader:GetPosition()) or inst:GetPosition()

        local list_guard = find_ents_at(guard_pos, guard_range)
        local list_self = find_ents_at(inst:GetPosition(), self_chase)
        candidates = intersect_lists(list_guard, list_self)
        ...
        if leader and leader:GetDistanceSqToPoint(guard_pos:Get()) <= (guard_range * guard_range) then
            ...
            candidates = intersect_lists(candidates, list_leader)
        end
    else
        -- 慎/攻：自己的追击距离 ∩ 主人的防御距离
        local list_self = find_ents_at(inst:GetPosition(), self_chase)
        local list_leader = leader and find_ents_near_leader_cached(leader, leader_def) or {}
        candidates = intersect_lists(list_self, list_leader)
    end
    ...
    for _, guy in ipairs(candidates) do
        if IsThreatToGuard(inst, guy) then
            ...
            local pri = 2
            if guy.components and guy.components.combat then
                if guy.components.combat:TargetIs(inst) then pri = 0
                elseif leader and guy.components.combat:TargetIs(leader) then pri = 0
                end
            end
