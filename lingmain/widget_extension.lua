...
-- 扩展 UIAnim 组件，添加 ScissorTo 功能
local _UIAnim_OnWallUpdate = UIAnim.OnWallUpdate

-- 添加 ScissorTo 相关方法到 UIAnim
function UIAnim:FinishCurrentScissor()
...
function UIAnim:CancelScissorTo(run_complete_fn)
    self.scissor_t = nil
...
function UIAnim:ScissorTo(start, dest, duration, whendone, curve)
    ...
    self.scissor_t = 0
    self.scissor_curve = curve or function(t) return t end -- 默认线性曲线
...
-- 扩展 OnWallUpdate 方法以支持 ScissorTo 动画
function UIAnim:OnWallUpdate(dt)
    -- 调用原始的 OnWallUpdate
    _UIAnim_OnWallUpdate(self, dt)

    if not self.inst:IsValid() then
		self.inst:StopWallUpdatingComponent(self, true)
		return
    end
...
-- 扩展 Widget 类，添加 ScissorTo 接口
function Widget:CancelScissorTo(run_complete_fn)
    if self.inst.components.uianim ~= nil then
        self.inst.components.uianim:CancelScissorTo(run_complete_fn)
    end
end

function Widget:ScissorTo(from, to, time, done, curve)
    if not self.inst.components.uianim then
        self.inst:AddComponent("uianim")
    end
    self.inst.components.uianim:ScissorTo(from, to, time, done, curve)
end

AddComponentPostInit("uianim", function(self)
    local _StopWallUpdatingComponent = self.inst.StopWallUpdatingComponent
    self.inst.StopWallUpdatingComponent = function(self, component, force)
        if force then
            _StopWallUpdatingComponent(self, component)
        end
    end
end)
